...
require_once('./includes/page.class.php'); //分页类
$m = new M(); //实例化 M 类
$total = $m->Total('tp_news'); //记录总数
$showrow = 2; //一页显示的行数
$curpage = empty($_GET['page']) ? 1 : $_GET['page']; //当前的页,还应该处理非数字的情况
$url = "?page={page}"; //分页地址，如果有检索条件 ="?page={page}&q=".$_GET['q']
//省略了链接mysql的代码，测试时自行添加
//$sql = "SELECT title, content, author, pubtime  FROM tp_news";
...
$data = $m->FetchAll("tp_news", "title, content, author, pubtime", "", "id DESC", $limit); //根据 M 类库中的 FetchAll 方法获取数据
?>
                    ...
                    <ul class="dates">
                        <?php foreach ($data as $v) {  ?>
                            <li>
                                ...
                                <a target="_blank" href="http://www.sucaihuo.com/js"><?php echo $v['title'] ?></a>
                            </li>
                        <?php } ?>
                    </ul>
                    <!--显示数据区-->
